...
def main():
    ...
    if args.run_seconds < 3:
        parser.error("--run-seconds must be at least 3.")
    ...
    stages = [("started", "Queue", 0.1), ("step", "Staging", 0.1), ("step", "Running", 0.8)]
    sequence = 0
    for event_type, state, fraction in stages:
        sequence += 1
        ...
        time.sleep(args.run_seconds * fraction)
